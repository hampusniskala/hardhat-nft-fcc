const { assert, expect } = require("chai")
const { network, deployments, ethers } = require("hardhat")
const { developmentChains } = require("../../helper-hardhat-config")

const lowSVGImageuri =
    "data:image/svg+xml;base64,"
const highSVGimageUri =
    "data:image/svg+xml;base64,"

const highTokenUri =
    "data:application/json;base64,"
const lowTokenUri =
    "data:application/json;base64,"

!developmentChains.includes(network.name)
    ? describe.skip
    : describe("Dynamic SVG NFT Unit Tests", function () {
          let dynamicSvgNft, deployer, mockV3Aggregator

          beforeEach(async () => {
              accounts = await ethers.getSigners()
              deployer = accounts[0]
              await deployments.fixture(["mocks", "dynamicsvg"])
              dynamicSvgNft = await ethers.getContract("DynamicSvgNft")
              mockV3Aggregator = await ethers.getContract("MockV3Aggregator")
          })

          describe("Construtor", () => {
              it("Initilizes the NFT and Values correctly.", async () => {
                  const name = await dynamicSvgNft.name()
                  const symbol = await dynamicSvgNft.symbol()
                  const tokenCounter = await dynamicSvgNft.getTokenCounter()
                  assert.equal(name, "Dynamic SVG NFT")
                  assert.equal(symbol, "DSN")
                  assert.equal(tokenCounter.toString(), "0")

                  const lowSVG = await dynamicSvgNft.getLowSVG()
                  const highSVG = await dynamicSvgNft.getHighSVG()
                  const priceFeed = await dynamicSvgNft.getPriceFeed()
                  assert.equal(lowSVG, lowSVGImageuri)
                  assert.equal(highSVG, highSVGimageUri)
                  assert.equal(priceFeed, mockV3Aggregator.address)
              })
          })

          describe("Mint NFT", () => {
              it("Allows users to mint an NFT, updates count and sends out an event", async function () {
                  const txResponse = await dynamicSvgNft.mintNft(1000)
                  await txResponse.wait(1)
                  const tokenCounter = await dynamicSvgNft.getTokenCounter()

                  assert.equal(tokenCounter.toString(), "1")
                  await expect(dynamicSvgNft.mintNft(1000)).to.emit(dynamicSvgNft, "CreatedNFT")
              })
          })

          describe("Token URI", () => {
              it("It changes the URI depending on priceFeed", async function () {
                  const veryHighValue = ethers.utils.parseEther("9999999").toString()
                  const veryLowValue = "1"

                  //Mint two NFTs
                  const txResponseMintOne = await dynamicSvgNft.mintNft(veryHighValue)
                  await txResponseMintOne.wait(1)
                  const txResponseMintTwo = await dynamicSvgNft.mintNft(veryLowValue)
                  await txResponseMintTwo.wait(1)

                  //Save their token URIs
                  const highValueURI = await dynamicSvgNft.tokenURI(1)
                  const lowValueURI = await dynamicSvgNft.tokenURI(2)

                  //Compare (when value is set high the price wont be above it so it will return the sad face/lowTokenUri)
                  assert.equal(highValueURI, lowTokenUri)
                  assert.equal(lowValueURI, highTokenUri)
              })
          })
          describe("Get Token Counter", () => {
              it("Gets the correct number of tokens", async function () {
                  const firstTokenCounter = await dynamicSvgNft.getTokenCounter()
                  const mintOne = await dynamicSvgNft.mintNft(1000)
                  await mintOne.wait(1)
                  const secondTokenCounter = await dynamicSvgNft.getTokenCounter()
                  const mintTwo = await dynamicSvgNft.mintNft(1000)
                  await mintTwo.wait(1)
                  const thirdTokenCounter = await dynamicSvgNft.getTokenCounter()

                  assert.equal(firstTokenCounter, "0")
                  assert.equal(secondTokenCounter, "1")
                  assert.equal(thirdTokenCounter, "2")
              })
          })
      })
